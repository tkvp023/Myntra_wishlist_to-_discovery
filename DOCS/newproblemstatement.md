...
## 8. Touchpoint Content Map (New — not in v1)
...
|---|---|---|:---:|
| **Notifications** | Single strongest/most-positive stat available | 1 stat | ❌ Only fires if a positive stat exists |
| **Homepage** | Same logic as Notifications | 1 stat (item count flexible, TBD by space) | ❌ Positive-only |
| **Search/Category browsing** | Trigger: keyword match, category match as fallback | Thumbnail + product name only, no stat | N/A — no stat shown |
| **Wishlist page** | Top 1–2 strongest stats | 1–2 stats | ❌ Positive-only |
| **Product page** | Shows everything | Full panel — all badges the product has data for, each with full state-by-state % breakdown | ✅ Complete, unfiltered data |

**Guiding principle:** the further a touchpoint is from the actual buying decision, the more curated/positive the signal shown; the closer to the decision point (Product page), the more complete and honest the data. This is a deliberate design pattern — worth stating explicitly if asked why some surfaces don't show negative data: it's not hidden, it's concentrated at the point of full transparency (Product page), while other surfaces act as positive highlights.

**Open items not yet resolved:**
- Exact copy variants for the 1–49 submission range, and the rule for when raw-count vs. momentum phrasing is used
- Tiebreak/display order when 2–3 wishlisted items match on Search/Category
- Dedupe logic between Homepage and Notifications (both use identical selection logic — risk of showing the same line twice)

**Explicitly out of scope, parked for later:** stock-scarcity messaging (e.g. "1 piece left") — a separate urgency lever, not part of this trust/quality-badge solution.

---

## 9. Notification Rules (Part C)
...
- Capped at 1 badge-based notification per item, sent once, triggered only when that item crosses the minimum-sample threshold while still in the user's wishlist
- If a price/discount notification and a badge notification would both fire for the same user on the same day, the price/discount send takes priority by default
