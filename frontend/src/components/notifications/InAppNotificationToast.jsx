...
export default function InAppNotificationToast({ wishlistItems = [] }) {
  ...
  const NOTIFICATION_TYPES = [
    {
      ...
      ctaText: 'View Verified Reviews',
      route: `/product/${product.id}?filter=${candidate.reengagement?.deepLinkFilter || 'authenticity'}`
    },
    {
      id: 'price_drop',
  ...
  return (
    ...
    >
      <div className="bg-white/98 backdrop-blur-md border-2 border-[#FF3F6C]/40 rounded-2xl p-3 shadow-xl flex items-center gap-3">
        
        {/* Thumbnail */}
        <img
          src={image}
          alt={product.name}
          className="w-11 h-13 object-cover object-top rounded-lg border border-[#EAEAEC] flex-shrink-0"
        />
        ...
        >
          ...
          <span className="text-[10px] font-bold text-[#FF3F6C] hover:underline inline-flex items-center gap-0.5 mt-0.5">
        ...
        <button
          ...
          className="p-1 rounded-full text-[#94969F] hover:text-[#282C3F] hover:bg-[#F5F5F6] cursor-pointer -mt-4 -mr-1"
        >
          <X className="w-4 h-4" />
        </button>
      </div>
